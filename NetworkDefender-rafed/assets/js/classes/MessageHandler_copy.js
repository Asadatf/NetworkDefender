...
class MessageHandler {
  ...
  visualizeCaesarCipherShift(word) {
    ...
    // Shift explanation
    const shiftExplanation = this.scene.add.text(
      centerX, 
      centerY - 50, 
    ...
    const originalWordText = this.scene.add.text(
      centerX, 
      centerY, 
      `Original: ${word}`, 
      {
    ...
    this.encryptedWordText = this.scene.add.text(
      centerX, 
      centerY + 50, 
      `Encrypted: ${word}`, 
      {
    ...
    this.cursorText = this.scene.add.text(
      centerX,
      centerY + 80,
      "^",
      {
    ...
    this.keydownListener = (event) => {
      if (this.shiftVisualizationObjects && this.shiftVisualizationObjects.length > 0) {
        switch(event.key) {
          case "ArrowLeft":
            // Move cursor left
          ...
          case "ArrowRight":
            // Move cursor right
            this.currentCharIndex = Math.min(this.currentEncryptWord.length - 1, this.currentCharIndex + 1);
            this.updateCursorPosition();
            break;
  ...
  updateCursorPosition() {
    ...
    const baseX = this.encryptedWordText.x - (this.encryptedWordText.width / 2) + prefixWidth;
  ...
  updateDisplay() {
    this.encryptedWordText.setText(`Encrypted: ${this.currentEncryptedWord.join('')}`);
    this.updateCursorPosition();
  }
  updateEncryptionVisualization(encryptedWordText) {
    // Update to properly display the current character
    const displayWord = this.currentEncryptWord.split('').map((char, index) => 
  ...
  if (char.match(/[a-zA-Z]/)) {
    ...
    const base = currentChar === currentChar.toLowerCase() ? 'a'.charCodeAt(0) : 'A'.charCodeAt(0);
    ...
    this.currentEncryptedWord[this.currentCharIndex] = String.fromCharCode(newCode);
  }
}
  ...
  if (!this.selectedWords.includes(selectedWord)) {
    if (this.selectedWords.length < 2) {
      ...
      if (this.selectedWords.length === 2) {
        ...
        this.wordTextObjects.forEach((wordText) => wordText.setVisible(false));
        this.showEncryptionMethodSelection();
        }
      }
    }
